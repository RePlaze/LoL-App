...
class SmoothBottomBar @JvmOverloads constructor(
    ...
    private fun tintAndDrawIcon(
        ...
        val isItemSelected = index == itemActiveIndex
        val iconTint = if (isItemSelected) itemIconTintActive else itemIconTint
    ...
    private fun onClickAction(viewId: Int) {
        ...
        if (viewId != itemActiveIndex) {
            ...
            // Update icon tint for reselected item
            for ((index, item) in items.withIndex()) {
                item.icon.mutate()
                DrawableCompat.setTint(
                    item.icon,
                    if (index == itemActiveIndex) Color.WHITE else itemIconTint
                )
            }
            invalidate()
